...
output_dir="$DILATED_CNN_NER_ROOT/data/$data_name-w$filter_width-$embeddings_name_$THRESHOLD"
...
mkdir -p $vocab_dir
# if train is a directory, then assume ontonotes/conll-2012/v4/data
if [ -d $raw_data_dir/${data_files[0]} ]; then
    cat `find $raw_data_dir/${data_files[0]} -type f -name \*_gold_conll | grep "english" | grep -v "/pt/nt/"` \
